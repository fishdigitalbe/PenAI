...
export async function exportAsHTML(result: GenerationResult, seoData?: any): Promise<void> {
  try {
    ...
    const htmlContent = generateSEOOptimizedHTML(result, seoData, jsonLdData);
    ...
    link.href = url;
    link.download = `${sanitizeFilename(result.title)}.html`;
...
function generateSEOOptimizedHTML(result: GenerationResult, seoData?: any, jsonLdData?: any): string {
  const resultSeo = (result as any).seo;

  const title = seoData?.metaTitle || resultSeo?.metaTitle || result.title;
  const description = seoData?.metaDescription || resultSeo?.metaDescription || result.metadata?.description || '';
  const keywords = seoData?.keywords || resultSeo?.keywords || result.metadata?.keywords || [];
  const h1Title = seoData?.h1 || resultSeo?.h1 || result.title;
  const ogTitle = seoData?.ogTitle || resultSeo?.ogTitle || title;
  const ogDescription = seoData?.ogDescription || resultSeo?.ogDescription || description;
  ...
  const publishDate = result.generatedAt;
...
<div class="chapterPage">
    ...
    <hr />
    <p class="highlight">
        ${escapeHtml(chapter.intro || chapter.content.split('\n')[0] || 'Hoofdstuk introductie')}
    </p>
...
<head>
  ...
  ${result.metadata?.jsonLd ? `<script type="application/ld+json">
